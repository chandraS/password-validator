...
def build_trie(root):
    ...
    my_parser = argparse.ArgumentParser(
        prog="password_validator", description="Read files and validate password"
    )
    my_parser.add_argument(
        "filepath", metavar="filepath", type=str, help="password file location"
    )
    ...
    if not os.path.isfile(password_file):
        raise ValueError(
            "The file specified does not exist. Please provide a valid path."
        )
    ...
    if not all(ord(c) < UPPER_ASCII_CODE and ord(c) > LOWER_ASCII_CODE for c in text):
        print(
            "".join(
                [
                    c
                    if ord(c) < UPPER_ASCII_CODE and ord(c) > LOWER_ASCII_CODE
                    else "*"
                    for c in text
                ]
            )
